...
bool has_srl_device_recv = false;
...
static usb_error_t handle_usb_event(usb_event_t event, void *event_data,
    ...
    if(event == USB_HOST_CONFIGURE_EVENT || (event == USB_DEVICE_ENABLED_EVENT && !(usb_GetRole() & USB_ROLE_DEVICE))) {
        ...
        if(has_srl_device_recv) return USB_SUCCESS;
...
void assign_string(string_t* os_in, char input[]){
    int length = strlen(input);
    for (int i=0; i<length; i++){
        os_in -> data[i] = input[i];
    }
}
...
int main(void){
    ...
    if(has_srl_device_recv){
        char input[44];
        
        string_t *os_in = ti_MallocString(44);
        assign_string(os_in, input);

        ti_SetVar(OS_TYPE_STR, OS_VAR_STR1, &os_in);

        srl_Read(&srl, input, 44);
        free(os_in);
    }
    else{
        //ti_SetVar(OS_TYPE_STR, OS_VAR_STR1);
    }
    kb_Scan();
    usb_Cleanup();
    return 0;
